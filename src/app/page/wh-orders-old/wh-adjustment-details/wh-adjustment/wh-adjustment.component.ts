...
import { WhAdjustmentDetailsComponent } from '../wh-adjustment-details/wh-adjustment-details.component';

@Component({
  selector: 'app-wh-adjustment',
  templateUrl: './wh-adjustment.component.html',
  styleUrls: ['./wh-adjustment.component.less'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class WHAdjustmentComponent implements OnInit, AfterViewInit {
  @ViewChild(MatAccordion) accordion: MatAccordion;
  @ViewChild(WhAdjustmentDetailsComponent, { static: false }) whAdjastmentDetails;
  myForm: FormGroup;
  id: any;
  order_type_id: any;
  brand_id: any;
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<WhAdjustmentDetailsComponent>,
  ...
  submit() {
    ...
    const endpoint= this.action === 'edit' ? 'wh_adjustment_details/post' : 'wh_adjustment_details/put';
    this.AC.post(this.myForm.value,endpoint).subscribe((answer: any) => {
      this.myForm.patchValue(
        {
          create_date: answer.wh_adjustment_details.create_date,
          id: Number(answer.wh_adjustment_details.id)
        });
      setTimeout(() => {
        this.onAdd.emit();
        this.changeRef.detectChanges();
  ...
  getTotalsReceivedId() {
    ...
    const endpoint= 'wh_adjustment_details/getTotalsReceivedId';
    this.AC.post(this.myForm.value,endpoint).subscribe((answer: any) => {
      this.total_ordered_qty = Number(answer?.wh_adjustment_details[0]?.total_received_qty);
      this.myForm.patchValue(
        {
          ordered_qty: Number(answer?.wh_adjustment_details[0]?.ordered_qty)
        });
    });
  }
  get TotalOrderedQty() {
  ...
  async getItemByID() {
    ...
    const endpoint= 'wh_adjustment_details/getById';
    this.AC.post(params,endpoint).subscribe((answer: any) => {
      this.myForm.patchValue(
        {
          w_order_id: Number(answer.wh_adjustment_details.w_order_id),
          product_id: Number(answer.wh_adjustment_details.product_id),
          old_qty: Number(answer.wh_adjustment_details.old_qty),
          new_qty: Number(answer.wh_adjustment_details.new_qty),
          create_by: answer.wh_adjustment_details.create_by,
          create_date: answer.wh_adjustment_details.create_date,
          notes: answer.wh_adjustment_details.notes,
          id: Number(answer.wh_adjustment_details.id)
        });
      this.selected_product_id = Number(answer.wh_adjustment_details.product_id);
      setTimeout(() => {
        this.onAdd.emit();
