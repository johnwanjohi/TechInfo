...
import { WhVanStockDetailsComponent } from '../wh-vanstock-details/wh-vanstock-details.component';

@Component({
  selector: 'app-wh-vanstock',
  templateUrl: './wh-vanstock.component.html',
  styleUrls: ['./wh-vanstock.component.less'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class WHVanStockComponent implements OnInit, AfterViewInit {
  @ViewChild(MatAccordion) accordion: MatAccordion;
  @ViewChild(WhVanStockDetailsComponent, { static: false }) whVanStockDetails;
  myForm: FormGroup;
  id: any;
  order_type_id: any;
  brand_id: any;
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<WhVanStockDetailsComponent>,
  ...
  submit() {
    ...
    const params = {
      ...
      //module: 'wh_vanstock_details',
      form: this.myForm.value,
    };
    //console.dir(params);
    const endpoint= this.action === 'edit' ? 'wh_vanstock_details/post' : 'wh_vanstock_details/put';
    this.AC.post(this.myForm.value,endpoint).subscribe((answer: any) => {
      this.myForm.patchValue(
        {
          create_date: answer.wh_vanstock_details.create_date,
          id: Number(answer.wh_vanstock_details.id)
        });
      setTimeout(() => {
        this.onAdd.emit();
        this.changeRef.detectChanges();
  ...
  getTotalsReceivedId() {
    ...
    const endpoint=  'wh_vanstock_details/getTotalsReceivedId';
    this.AC.post(this.myForm.value,endpoint).subscribe((answer: any) => {
      this.total_ordered_qty = Number(answer?.wh_vanstock_details[0]?.total_received_qty);
      this.myForm.patchValue(
        {
          ordered_qty: Number(answer?.wh_vanstock_details[0]?.ordered_qty)
        });
    });
  }
  get TotalOrderedQty() {
  ...
  async getItemByID() {
    ...
    const endpoint=  'wh_vanstock_details/getById';
    this.AC.post(params,endpoint).subscribe((answer: any) => {
      this.myForm.patchValue(
        {
          w_order_id: Number(answer.wh_vanstock_details.w_order_id),
          product_id: Number(answer.wh_vanstock_details.product_id),
          issue_qty: Number(answer.wh_vanstock_details.issue_qty),
          create_by: answer.wh_vanstock_details.create_by,
          create_date: answer.wh_vanstock_details.create_date,
          notes: answer.wh_vanstock_details.notes,
          id: Number(answer.wh_vanstock_details.id)
        });
      this.selected_product_id = Number(answer.wh_vanstock_details.product_id);
      setTimeout(() => {
        this.onAdd.emit();
